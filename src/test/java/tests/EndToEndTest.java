...
public class EndToEndTest extends BaseTest {
    ...
    public void e2eapirequest() throws IOException {
        ...
        String patchApiRequestBody= FileUtils.readFileToString(new File(Constants.Patch_API_Request_Body),"UTF-8");
        ...
        //get api call will pass the booking id as the header
        ...
        //patch api call
        RestAssured.given().contentType(ContentType.JSON).header("Cookie","token="+token).baseUri("https://restful-booker.herokuapp.com/booking").
                body(patchApiRequestBody).when().patch("/{bookingId}",bookingid1).then().assertThat().statusCode(200).body("firstname",Matchers.equalTo("Testers Talk"))
                .body("lastname",Matchers.equalTo("Selenium C#"))
        ;
        //delete api call
        RestAssured.given().contentType(ContentType.JSON).header("Cookie","token="+token).baseUri("https://restful-booker.herokuapp.com/booking").
                body(patchApiRequestBody).when().delete("/{bookingId}",bookingid1).then().assertThat().statusCode(201)
        ;
